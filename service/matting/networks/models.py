...
def build_model(
    ...
    if weights != "default":
        ...
        if not os.path.exists(dir_weights + weights):
            os.makedirs(dir_weights, exist_ok=True)
            gdown.download(url, dir_weights + weights, quiet=False)

        sd = torch.load(dir_weights + weights, map_location=device)
...
class ModelBuilder:
    def build_encoder(self, arch="resnet50_GN"):
        if arch == "resnet50_GN_WS":
            orig_resnet = l_resnet50_gn_ws()
            net_encoder = ResnetDilated(orig_resnet, dilate_scale=8)

        elif arch == "resnet50_BN":
            orig_resnet = l_resnet50_bn()
        ...
        for scale in pool_scales:
            self.ppm.append(
                nn.Sequential(
                    ...
                    Conv2d(2048, 256, kernel_size=1, bias=True),
